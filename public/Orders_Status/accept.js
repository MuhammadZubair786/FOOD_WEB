            ...
            value.map((v, i) => {
                ...
                <img src=${v.item_img} class="card-img-top" alt=${v.item_name} style='width:95%;margin-top:10px;margin-left:10px;margin-right:10px;height:250px'> 
                    <div class="card-body">
                        ...
                        <a href="#" class="btn btn-success" id=${v.key} onclick=delievered_order(this)>Accept</a>
                    </div>
                </div>
            </div>`
                      }  })
                ...
                if(order==null){
                    ...
                    data2.map((v,i)=>{
                        ...
                        if(v.key == e.id){
                            ...
                            console.log(obj)
                            firebase.database().ref(`/Resturant/${v.resturant_uid}`).child(`Dishes/${v.key}/Order/${v.customer_uid}`).set(obj)
                            firebase.database().ref(`/User`).child(`${v.customer_uid}/Order/${v.key}`).set(obj)
                            .then((data)=>{
